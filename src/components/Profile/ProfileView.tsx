...
const StatCard = ({ label, value }: { label: string; value: string }) => (
    <div className="bg-cream p-4 rounded-lg">
        <h3 className="text-xs sm:text-sm font-semibold text-secondary-green mb-1 uppercase tracking-wide">{label}</h3>
        <p className="text-lg sm:text-xl font-bold text-primary-green capitalize">{value}</p>
    </div>
)
    ...
    return (
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="bg-primary-green p-4 sm:p-6 md:p-8">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                        <h1 className="text-2xl sm:text-3xl font-bold text-cream mb-1">{profile.displayName}</h1>
                    </div>
                    <button
                        onClick={onEdit}
            ...
            <div className="p-4 sm:p-6 md:p-8">
                {profile.bio && (
                    <div className="mb-6">
                        <h2 className="text-lg sm:text-xl font-bold text-primary-green mb-2">About</h2>
                        <p className="text-secondary-green leading-relaxed">{profile.bio}</p>
                    </div>
                )}
